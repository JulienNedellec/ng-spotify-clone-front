  ...
  constructor() {
    effect(() => {
      ...
      if(this.songService.addSig().status === "OK") {
        ...
        this.toastService.show({ body: 'Song created with success', type: "SUCCESS" });
        this.router.navigate(['/']);
      } else if (this.songService.addSig().status === "ERROR") {
        this.toastService.show({ body: 'Error occured when creating song, please try again', type: "DANGER" });
      }
    });
  }
