      ...
      .subscribe({
        next: updatedSong => {
          ...
          if(updatedSong.favorite) {
            this.toastService.show({ body: 'Song added to favorite', type: 'SUCCESS' });
          } else {
            this.toastService.show({ body: 'Song removed from favorite', type: 'SUCCESS' });
          }
        } ,
        error: err => {
          this.addOrRemoveFavoriteSong$.set(State.Builder<ReadSong, HttpErrorResponse>().forError(err).build());
          this.toastService.show({ body: 'Error when adding song to favorite', type: 'DANGER' });
        }
      })
  }
